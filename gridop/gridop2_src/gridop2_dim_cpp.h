...
int main1(int argc, char* argv[])
{
    ...
    const length_type
        ...
        x2 = (x0 + x1) * 0.5,
        h = L / double(N); // [m]
    const time_type dt = 3e-12 * si::second; // [s]
    const velocity_type CS = 500 * si::meters_per_second; // [m/s]
    const energy_type CS2 = CS * CS;
    const time_type tau = 0.5 * h / CS; //[s]
    const typename units::multiply_typeof_helper<velocity_type, length_type>::type eta = 5e-4 * 2 * (si::meters_per_second * si::meter); // [Pa*s]
    const units::quantity<units::unit<typename units::derived_dimension<units::length_base_dimension, 4, units::time_base_dimension, -2>::type, si::system> >
        lambda1 = (2e-4 * 2) * (si::meters_per_second * si::meters_per_second * si::meter * si::meter);
    //const energy_type lambda2 = (2e-4 * 2) * (si::meters_per_second * si::meters_per_second);
    const time_type M = 2e-8 * si::second; //7e-8,
    const energy_type A_psi = (2e+4 * 2) * (si::meters_per_second * si::meters_per_second);
    typedef units::quantity<typename units::divide_typeof_helper<si::dimensionless, si::length>::type> b_type;
    const b_type b = func::sqrt(2.0 * A_psi / lambda1);
    ...
    cn = initializer<tag_cntr, nodim, length_type, b_type>(CNTR_N, CNTR_N, x0, x1, x2, y0, y1, y2, h, b, R);
//#ifdef __CUDACC__
//    HOST_VECTOR_T<nodim> hcn(cn.size());
//#else
//    HOST_VECTOR_T<nodim>& hcn = cn;
//#endif
...
//#endif
    for (int iter = 0; iter < max_iter /*true*/; iter++) { // infinite cycle
        ...
        if (iter % (max_iter / 10) == 0)
            ...
            << MATH_ACCUMULATE(totEnrg.begin(), totEnrg.end(), 0.0 * (si::meters_per_second * si::meters_per_second)).value() << std::endl;
