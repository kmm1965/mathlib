...
template<typename F>
struct math_binder1st : public math_unary_function<typename F::second_argument_type, typename F::result_type>
{
	typedef math_unary_function<typename F::second_argument_type, typename F::result_type> super;
	typedef typename super::argument_type argument_type;
	typedef typename super::result_type result_type;

	__device__ __host__
	math_binder1st(F bin_op_, const typename F::first_argument_type &left_) : bin_op(bin_op_), left(left_){}
...
protected:
	F bin_op;
	typename F::first_argument_type left;
};

template<typename F>
__device__ __host__
math_binder1st<F> math_bind1st(F bin_op, const typename F::first_argument_type &left){
	return math_binder1st<F>(bin_op, left);
}

template<typename F>
struct math_binder2nd : public math_unary_function<typename F::first_argument_type, typename F::result_type>
{
	typedef math_unary_function<typename F::first_argument_type, typename F::result_type> super;
	typedef typename super::argument_type argument_type;
	typedef typename super::result_type result_type;

	__device__ __host__
	math_binder2nd(F bin_op_, const typename F::second_argument_type &right_) : bin_op(bin_op_), right(right_){}
...
protected:
	F bin_op;
	typename F::second_argument_type right;
};

template<typename F>
__device__ __host__
math_binder2nd<F> math_bind2nd(F bin_op, const typename F::second_argument_type &right){
	return math_binder2nd<F>(bin_op, right);
}
