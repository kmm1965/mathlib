...
#define DECLARE_MONAD_CLASS(M, _M) \
	template<typename T> \
    static M<fdecay<T> > mreturn(T const& x); \
...
#define IMPLEMENT_MRETURN(M, _M) \
	template<typename T> \
    M<fdecay<T> > Monad<_M>::mreturn(T const& x) { return super::pure(x);  }
...
template<typename M, typename MFUNC>
mbind_type<M, MFUNC> operator>>=(M const& m, MFUNC const& f){
    return Monad_t<M>::mbind(m, f);
}

template<typename M, typename MFUNC>
mbind_type<M, MFUNC> operator>>=(f0<M> const& mf, MFUNC const& f){
    return Monad_t<M>::mbind(*mf, f);
}

template<typename M, typename MFUNC>
mbind_type<M, MFUNC> operator<<=(MFUNC const& f, M const& m){
	return m >>= f;
}

template<typename M, typename MFUNC>
mbind_type<M, MFUNC> operator<<=(MFUNC const& f, f0<M> const& mf){
	return *mf >>= f;
}
...
template<typename MV, typename MF>
ap_type<MV, MF> operator&(MF const& mf, MV const& mv) {
	return ap(mf, mv);
}
