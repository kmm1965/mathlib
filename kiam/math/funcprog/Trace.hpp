...
*/
void traceIO(const char* s){
    std::cout << s << std::endl;
}

void traceIO(std::string const& s){
    traceIO(s.c_str());
}
...
*/
void putTraceMsg(const char* s){
    traceIO(s);
}

void putTraceMsg(std::string const& s){
    putTraceMsg(s.c_str());
}
...
*/
template<typename A>
A trace(const char* s, A const& a)
{
    traceIO(s);
    return a;
}
...
template<typename A>
A trace(std::string const& s, A const& a){
    return trace(s.c_str(), a);
}
...
traceId a = trace a a
*/
const char* traceId(const char* s){
    return trace(s, s);
}

std::string traceId(std::string const& s){
    return trace(s.c_str(), s);
}
...
B traceShow(A const& a, B const& b){
    return trace(show(a), b);
}
...
constexpr auto traceM(const char *s){
    return trace(s, Applicative<_F>::pure(None()));
}

template<typename _F>
constexpr auto traceM(std::string const& s){
    return traceM<_F>(s.c_str());
}
...
*/
template<typename _F, typename A>
constexpr auto traceShowM(A const& a){
    return traceM<_F>(show(a));
}
