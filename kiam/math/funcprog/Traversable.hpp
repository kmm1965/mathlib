...
#define DECLARE_TRAVERSABLE_CLASS(T) \
    ...
    static typename std::enable_if<is_applicative_t<AP>::value, typeof_t<AP, T<value_type_t<AP> > > >::type \
...
using traverse_type = typename std::enable_if<
    is_traversable<T>::value && is_applicative_t<AP>::value && is_same_as<value_type_t<T>, Arg>::value,
...
using sequenceA_type = typename std::enable_if<
    is_traversable<T>::value && is_applicative_t<value_type_t<T> >::value,
...
using mapM_type = typename std::enable_if<
    is_traversable<T>::value && is_monad_t<M>::value && is_same_as<value_type_t<T>, ARG>::value,
...
using sequence_type = typename std::enable_if<
    is_traversable<T>::value && is_monad_t<value_type_t<T> >::value,
    typeof_t<value_type_t<T>, typeof_t<T, value_type_t<value_type_t<T> > > >
>::type;
