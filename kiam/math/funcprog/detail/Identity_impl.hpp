...
constexpr Identity<remove_f0_t<function_t<Ret(Args...)> > >
Functor<_Identity>::fmap(function_t<Ret(Arg, Args...)> const& f, Identity<fdecay<Arg> > const& v){
    return invoke_f0(f << v.run());
}

// Applicative
template<typename T>
constexpr Identity<fdecay<T> > Applicative<_Identity>::pure(T const& x) {
    return x;
}
...
template<typename Ret, typename Arg, typename... Args>
constexpr remove_f0_t<function_t<Identity<Ret>(Args...)> >
