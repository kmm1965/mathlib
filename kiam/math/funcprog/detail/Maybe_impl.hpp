...
constexpr Functor<_Maybe>::fmap(function_t<Ret(Arg, Args...)> const& f, Maybe<fdecay<Arg> > const& v)
{
...
// pure = Just
template<typename T>
constexpr Maybe<fdecay<T> > Applicative<_Maybe>::pure(T const& x){
    return Just(x);
}

template<typename Ret, typename Arg, typename... Args>
Maybe<remove_f0_t<function_t<Ret(Args...)> > >
constexpr Applicative<_Maybe>::apply(Maybe<function_t<Ret(Arg, Args...)> > const& f, Maybe<fdecay<Arg> > const& v){
...
constexpr Monad<_Maybe>::mbind(Maybe<fdecay<Arg> > const& m, function_t<Maybe<Ret>(Arg, Args...)> const& f)
{
    using result_type = Maybe<remove_f0_t<function_t<Ret(Args...)> > >;
...
// empty = Nothing
template<typename A>
constexpr Maybe<A> Alternative<_Maybe>::empty(){
    return Nothing<A>();
}
...
// l       <|> _ = l
template<typename A>
constexpr Maybe<A> Alternative<_Maybe>::alt_op(Maybe<A> const& l, Maybe<A> const&r){
    return l ? l : r;
}

// Semigroup
template<typename A>
constexpr semigroup_type<A, Maybe<A> > Semigroup<_Maybe>::semigroup_op(Maybe<A> const& x, Maybe<A> const& y){
    return !x ? y : !y ? x : Just(x.value() % y.value());
}

template<typename A>
constexpr semigroup_type<A, Maybe<A> > Semigroup<_Maybe>::stimes(int n, Maybe<A> const& m)
{
    assert(n >= 0);
...
// throwError () = Nothing
template<typename A>
constexpr Maybe<A> MonadError<_Maybe>::throwError(error_type<A> const&){
    return Nothing<A>();
}
...
// catchError x       _ = x
template<typename A>
constexpr Maybe<A> MonadError<_Maybe>::catchError(Maybe<A> const& x, function_t<Maybe<A>(error_type<A> const&)> const& f){
