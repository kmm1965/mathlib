...
constexpr auto operator^(function_t<RetF(ArgsF...)> const& f, function_t<RetG(ArgG)> const& g)
{
...
template<typename MFUNC>
using mbind_arg_type = typeof_t<result_type_t<MFUNC>, first_argument_type_t<MFUNC> >;
...
template<typename MFUNC>
constexpr auto operator>>=(f0<mbind_arg_type<MFUNC> > const& mf, MFUNC const& f){
    return *mf >>= f;
}
...
template<typename MFUNC>
constexpr auto operator<<=(MFUNC const& f, f0<mbind_arg_type<MFUNC> > const& mf){
    return *mf >>= f;
}
...
constexpr auto mcompose(function_t<MF(ArgF, ArgsF...)> const& f, function_t<MG(ArgG, ArgsG...)> const& g)
{
...
template<typename MG, typename ArgG, typename... ArgsG, typename MF, typename ArgF, typename... ArgsF>
constexpr auto _mcompose(function_t<MF(ArgF, ArgsF...)> const& f)
{
    static_assert(is_same_monad_v<MF, MG> && is_same_as_v<value_type_t<MF>, ArgG>, "Should be the same monad");
    return _([f](function_t<MG(ArgG, ArgsG...)> const& g){
        return mcompose(f, g);
    });
}
