...
FUNCTION_TEMPLATE_ARGS(3) constexpr remove_f0_t<function_t<T2(Args...)> > either(function_t<T2(T0, Args...)> const& f,
    function_t<T2(T1, Args...)> const& g, EITHER(fdecay<T0>, fdecay<T1>) const& x)
{
...
constexpr Either<A, remove_f0_t<function_t<Ret(Args...)> > >
...
template<typename A>
template<typename T>
constexpr Either<A, fdecay<T> >
Applicative<_Either<A> >::pure(T const& x){
    return Right(x);
}
...
constexpr remove_f0_t<function_t<Either<A, C>(Args...)> >
...
template<typename A>
template<typename B>
constexpr Either<A, B>
MonadError<_Either<A> >::throwError(A const& x){
    return _Left<A>(x);
}
...
template<typename B>
constexpr Either<A, B>
MonadError<_Either<A> >::catchError(Either<A, B> const& x, function_t<Either<A, B>(A const&)> const& f){
    return x.index() == Left_ ? f(*(x.left())) : x;
}

// Functor
template<typename Ret, typename Arg, typename... Args>
constexpr Either<void, remove_f0_t<function_t<Ret(Args...)> > >
...
// Applicative
template<typename T>
constexpr Either<void, fdecay<T> >
Applicative<_Either<void> >::pure(T const& x){
    return Right(x);
}
...
// Monad
template<typename C, typename Arg, typename... Args>
constexpr remove_f0_t<function_t<Either<void, C>(Args...)> >
