...
*/
template<typename S, typename U, typename M, typename Ret, typename T, typename P>
struct parsecMap_unParser
{
    using ParsecT_base_t = ParsecT_base<S, U, M, Ret>;

    parsecMap_unParser(function_t<Ret(T const&)> const& f, ParsecT<S, U, M, T, P> const& p) : f(f), p(p) {}

    IMPLEMENT_UNPARSER_RUN(return p.template run<B>(s, cok & f, cerr, eok & f, eerr);)

private:
    function_t<Ret(T)> const f;
    ParsecT<S, U, M, T, P> const p;
};

#define PARSECMAP_UNPARSER_(S, U, M, Ret, T, P) BOOST_IDENTITY_TYPE((_PARSEC::parsecMap_unParser<S, U, M, Ret, T, P>))
...
using parsec_fmap_type = parsec::ParsecT<S, U, M, Ret, parsec::parsecMap_unParser<S, U, M, Ret, fdecay<Arg>, P> >;

#define PARSEC_FMAP_TYPE_(S, U, M, Ret, Arg, P) BOOST_IDENTITY_TYPE((parsec_fmap_type<S, U, M, Ret, Arg, P>))
