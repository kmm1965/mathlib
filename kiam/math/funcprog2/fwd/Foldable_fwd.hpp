...
DECLARE_FUNCTION_2(4, FOLDMAP_TYPE(T0, T1, T2), foldMap, FUNCTION2(T1(T2), T3) const&, T0 const&);
...
DECLARE_FUNCTION_3(5, FOLDLR_TYPE(T0, T1, T2, T3), foldl, FUNCTION2(T3(T2, T1), T4) const&, T3 const&, T0 const&);

// foldl1 :: (a -> a -> a) -> t a -> a
DECLARE_FUNCTION_2(5, fold1_type<T0>, foldl1, FUNCTION2(T3(T1, T2), T4) const&, T0 const&);

// foldr :: (a -> b -> b) -> b -> t a -> b
//DECLARE_FUNCTION_3(5, FOLDLR_TYPE(T0, T1, T2, T3), foldr, FUNCTION2(T3(T1, T2), T4) const&, T3 const&, T0 const&);
FUNCTION_TEMPLATE(5) __DEVICE constexpr FOLDLR_TYPE(T0, T1, T2, T3) foldr(function2<T3(T1, T2), T4> const&, T3 const&, T0 const&);

// foldr1 :: (a -> a -> a) -> t a -> a
DECLARE_FUNCTION_2(5, fold1_type<T0>, foldr1, FUNCTION2(T3(T1, T2), T4) const&, T0 const&);
...
template<typename F, typename Arg, typename FuncImpl>
constexpr foldable_type<F, bool> any(function2<bool(Arg), FuncImpl> const& p, F const& f);

template<typename F, typename Arg, typename FuncImpl>
constexpr auto _any(function2<bool(Arg), FuncImpl> const& p)
{
...
template<typename F, typename Arg, typename FuncImpl>
constexpr foldable_type<F, bool> all(function2<bool(Arg), FuncImpl> const& p, F const& f);

template<typename F, typename Arg, typename FuncImpl>
constexpr auto _all(function2<bool(Arg), FuncImpl> const& p)
{
    static_assert(is_foldable_v<F>, "Should be Foldable");
...
template<typename F, typename Arg1, typename Arg2, typename FuncImpl>
constexpr foldable_type<F, value_type_t<F> >
maximumBy(function2<Ordering(Arg1, Arg2), FuncImpl> const& cmp, F const& f);

template<typename F, typename Arg1, typename Arg2, typename FuncImpl>
constexpr auto _maximumBy(function2<Ordering(Arg1, Arg2), FuncImpl> const& cmp)
{
    static_assert(is_foldable_v<F>, "Should be Foldable");
...
template<typename F, typename Arg1, typename Arg2, typename FuncImpl>
constexpr foldable_type<F, value_type_t<F> >
minimumBy(function2<Ordering(Arg1, Arg2), FuncImpl> const& cmp, F const& f);

template<typename F, typename Arg1, typename Arg2, typename FuncImpl>
constexpr auto _minimumBy(function2<Ordering(Arg1, Arg2), FuncImpl> const& cmp)
{
    static_assert(is_foldable_v<F>, "Should be Foldable");
...
//find(function2<bool(Arg), FuncImpl> const& p, F const& f);
//
//template<typename F, typename Arg, typename FuncImpl>
//constexpr foldable_type<F, function2<Maybe<value_type_t<F> >(F const&)> >
//_find(function2<bool(Arg), FuncImpl> const& p)
//{
//    return _([p](F const& f) { return find(p, f); });
//}

DECLARE_FUNCTION_3(5, FOLDABLE_TYPE(T0, T1), foldrM, FUNCTION2(T1(T2, T3), T4) const&, value_type_t<T1> const&, T0 const&);
DECLARE_FUNCTION_3(5, FOLDABLE_TYPE(T0, T1), foldlM, FUNCTION2(T1(T3, T2), T4) const&, value_type_t<T1> const&, T0 const&);

DECLARE_FUNCTION_2(4, FOLDABLE_TYPE(T0, TYPEOF_T(T1, None)), traverse_, FUNCTION2(T1(T2), T3) const&, T0 const&);
DECLARE_FUNCTION_2(4, FOLDABLE_TYPE(T0, TYPEOF_T(T1, None)), for_, T0 const&, FUNCTION2(T1(T2), T3) const&);

DECLARE_FUNCTION_2(4, FOLDABLE_TYPE(T0, TYPEOF_T(T1, None)), mapM_, FUNCTION2(T1(T2), T3) const&, T0 const&);
DECLARE_FUNCTION_2(4, FOLDABLE_TYPE(T0, TYPEOF_T(T1, None)), forM_, T0 const&, FUNCTION2(T1(T2), T3) const&);
...
DECLARE_FUNCTION_2(4, FOLDABLE_TYPE(T0, List<T1>), concatMap, FUNCTION2(List<T1>(T2), T3) const&, T0 const&);
