...
struct operator_evaluator : evaluable_object<typename MO::template get_tag_type<typename EO::tag_type>::type, operator_evaluator<MO, EO> >
{
    ...
    CONSTEXPR operator_evaluator(const MATH_OP(MO) &op, const EOBJ(EO)&eobj) : op_proxy(op.get_proxy()), eobj_proxy(eobj.get_proxy()){}
...
#define MATH_OPERATOR_EVALUATOR(z, n, unused) \
    ...
    { \
	    ...
	    operator_evaluator##n(const MATH_OP(MO) &op, BOOST_PP_ENUM(n, MATH_OPERATOR_EVALUATOR_EOBJ, ~)) : \
...
#define REIMPLEMENT_OPERATOR_EXEC() \
    ...
    __DEVICE \
    CONSTEXPR typename super::template get_value_type<_KIAM_MATH::get_value_type_t<EOP> >::type operator()(size_t i, EOP const& eobj_proxy) const { \
        return super::operator()(i, eobj_proxy); \
    }

#define IMPLEMENT_MATH_EVAL_OPERATOR(type) \
	template<class EO> \
	CONSTEXPR _KIAM_MATH::operator_evaluator<type, EO> operator()(const EOBJ(EO) &eobj) const { \
		return operator_evaluator<type, EO>(*this, eobj); \
	}

#define IMPLEMENT_MATH_EVAL_OPERATOR_EOBJ(z, n, unused) \
