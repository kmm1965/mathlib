...
struct unstructured_grid_proxy
{
    ...
    constexpr unstructured_grid_proxy(grid_type const& grid) :
    ...
    IMPLEMENT_DEFAULT_COPY_CONSRUCTOR(unstructured_grid_proxy);
    ...
    vector_t const m_ptMin, m_ptMax;    // Минимальные и максимальные координаты вершин.
    vector_t const m_dxMin;             // Минимальные размеры ячеек.
#if UGRID_CELL_H
    value_type const m_hMin;            // Минимальный характерный размер ячейки.
#endif
    unsigned const m_local_cells, m_global_cells, m_local_nodes, m_global_nodes;
};
