...
void unstructured_grid<T, DIM>::calc_geometry(const mpi::communicator &comm)
{
    ...
    for(unsigned icell = 0; icell < cells.size(); ++icell){
        ...
        // Центр масс ячейки
        cell.mcenter = cell_mcenter<dim>(nodes.get_vector_proxy(), cell_node_index.get_vector_proxy(), cell_node_index_start, cell_node_count);
        // Объём ячейки
        cell.volume = cell_volume<dim>(nodes.get_vector_proxy(), cell_node_index.get_vector_proxy(), cell_node_index_start, cell_node_count);
        assert(cell.volume > 0);
    }
    comm.barrier();
    t_cells_geometry.stop();
    if(rank == 0)
        std::cout << t_cells_geometry.format(2, "done, time=%w seconds") << std::endl;
    // Рассчитаем геометрию граней.
    if(rank == 0)
        std::cout << "Calculating interfaces geometry..." << std::flush;
    boost::timer::cpu_timer t_interfaces_geometry;
    ...
    for(unsigned iface = 0; iface < interfaces.size(); ++iface){
        ...
        // Площадь грани
        interface.area = calc_interface_area<dim>(nodes.get_vector_proxy(), interface_node_index.get_vector_proxy(), interface_node_index_start, interface_node_count);
#endif
        ...
        if(interface_node_count == dim){
            ...
            interface.mcenter = quadrilateral_mcenter<dim, value_type>(
...
#endif
        // Внешнаяя нормаль к грани
        interface.normal = calc_normal<dim>(nodes.get_vector_proxy(), interface_node_index.get_vector_proxy(), interface_node_index_start, interface_node_count);
    ...
    comm.barrier();
    t_interfaces_geometry.stop();
    if(rank == 0)
        std::cout << t_interfaces_geometry.format(2, "done, time=%w seconds") << std::endl;
#if UGRID_CELL_H
    // Найдём минимальную высоту каждой ячейки
    if(rank == 0)
        std::cout << "Calculating minimal cell height..." << std::flush;
    boost::timer::cpu_timer t_min_cell_height;
    ...
    for(unsigned icell = 0; icell < cells.size(); ++icell){
        ...
        if(cell_interface_count == dim + 1){ // Тетраэдр или треугольник
            ...
            for(unsigned i = cell_interface_index_start; i < cell_interface_index_end; ++i){
                unsigned const iface = cell_interface_index[i];
                interface_value_type const& interface = interfaces[iface];
#if UGRID_INTERFACE_AREA
            ...
            cell.h = calc_cell_height<dim>(nodes.get_vector_proxy(), cell_node_index.get_vector_proxy(), cell_node_index_start, cell_node_shift[icell + 1] - cell_node_index_start);
        }
        assert(cell.h > 0);
        if(hMin > cell.h)
            hMin = cell.h;
    ...
    t_min_cell_height.stop();
    if(rank == 0)
        std::cout << "done, hMin=" << std::scientific << std::setprecision(3) << m_hMin << t_min_cell_height.format(2, ", time=%w seconds") << std::endl;
#endif // UGRID_CELL_H
